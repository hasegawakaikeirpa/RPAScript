...
def ZaisanUpdate(Job, Exc):
    ...
    try:
        ...
        if str(Exc.row_data["関与先番号"]) == ThisNo:
            ...
            while pg.locateOnScreen(URL + r"\ZaisanKousin.png", confidence=0.9) is None:
                time.sleep(1)
                CDQ = RPA.ImgCheck(
                    URL,
                    r"ZChangeDataQ.png",
            ...
            while pg.locateOnScreen(URL + r"\DataInIcon.png", confidence=0.9) is None:
                time.sleep(1)
            ME = RPA.ImgCheckForList(URL, [r"\MenuEnd.png", r"\MenuEnd2.png"], 0.9, 10)
            if ME[0] is True:
            ...
            # 財産評価メニューが表示されるまで待機------------------------------------
            while (
                pg.locateOnScreen(URL + r"\ZaisanOpenFlag.png", confidence=0.9) is None
            ):
                time.sleep(1)
            # --------------------------------------------------------------------
            time.sleep(3)
            ...
            # 更新区分フラグが表示されるまで待機------------------------------------
            while (
                pg.locateOnScreen(URL + r"\KousinkubunFlag.png", confidence=0.9) is None
            ):
                time.sleep(1)
            # --------------------------------------------------------------------
            ...
            if ErrStr == "":
                ...
                if not ErrStr == "NoData":
                    RPA.ImgClick(URL, r"\ZaisanStart.png", 0.9, 10)  # 更新開始のアイコンをクリック
                    # 確認ウィンドウが表示されるまで待機-------------------------------------
                    while (
                        pg.locateOnScreen(URL + r"\ZaisanStartQ.png", confidence=0.9)
                        is None
                    ):
                        time.sleep(1)
                    # --------------------------------------------------------------------
