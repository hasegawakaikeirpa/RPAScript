...
def NencyouUpdate(Job, Exc):
    ...
    try:
        ...
        if str(Exc.row_data["関与先番号"]) == ThisNo:
            ...
            # 年調更新アイコンが表示されるまで待機--------------------------------
            while (
                pg.locateOnScreen(URL + r"\NencyouOpenFlag.png", confidence=0.9) is None
            ):
                time.sleep(1)
                # 顧問先情報変更ダイアログが表示されたら
                CDQ = RPA.ImgCheck(
            ...
            while (
                pg.locateOnScreen(URL + r"\NencyouKakuteiQ.png", confidence=0.9) is None
            ):
                time.sleep(1)
            time.sleep(1)
            NK = RPA.ImgCheck(URL, r"\NencyouKubun.png", 0.99999, 10)  # 年調チェックボックスをクリック
            if NK[0] is True:
            ...
            # 更新確認ダイアログが表示されるまで待機--------------------------------
            while (
                pg.locateOnScreen(URL + r"\KousinKakunin.png", confidence=0.9) is None
            ):
                time.sleep(1)
            pg.press("y")
            # 更新完了ダイアログが表示されるまで待機--------------------------------
            while (
                pg.locateOnScreen(URL + r"\KousinKanryou.png", confidence=0.9) is None
            ):
                time.sleep(1)
            pg.press("return")
            # 取り込むボタンが表示されるまで待機--------------------------------
            ...
            # 印刷更新タブが表示されるまで待機--------------------------------
            while (
                pg.locateOnScreen(URL + r"\NencyouTopMenu.png", confidence=0.9) is None
            ):
                time.sleep(1)
            RPA.ImgClick(URL, r"\NencyouTopMenu.png", 0.9, 10)  # 印刷更新タブをクリック
            # 年調メニューが表示されるまで待機------------------------------------
            while (
                pg.locateOnScreen(URL + r"\NencyouKousin.png", confidence=0.9) is None
            ):
                time.sleep(1)
            # --------------------------------------------------------------------
            time.sleep(1)
            ...
            if ErrStr == "":
                ...
                # 確認ウィンドウが表示されるまで待機-------------------------------------
                while pg.locateOnScreen(URL + r"\NencyouQ.png", confidence=0.9) is None:
                    time.sleep(1)
                # --------------------------------------------------------------------
                pg.press("y")  # yで決定(nがキャンセル)
                # 確認ウィンドウが表示されるまで待機-------------------------------------
                while (
                    pg.locateOnScreen(URL + r"\NencyouQ2.png", confidence=0.9) is None
                ):
                    time.sleep(1)
                # --------------------------------------------------------------------
                pg.press("y")  # yで決定(nがキャンセル)
                # 処理終了ウィンドウが表示されるまで待機----------------------------------
                while (
                    pg.locateOnScreen(URL + r"\NencyouFQ.png", confidence=0.9) is None
                ):
                    time.sleep(1)
