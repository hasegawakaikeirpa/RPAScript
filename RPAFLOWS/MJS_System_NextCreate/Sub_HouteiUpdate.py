...
def HouteiUpdate(Job, Exc):
    ...
    try:
        ...
        if str(Exc.row_data["関与先番号"]) == ThisNo:
            ...
            # 法定調書メニューが表示されるまで待機--------------------------------
            while pg.locateOnScreen(URL + r"\HouteiMenu.png", confidence=0.9) is None:
                time.sleep(1)
                # 顧問先情報変更ダイアログが表示されたら
                CDQ = RPA.ImgCheck(
                    URL,
            ...
            # 取り込むボタンが表示されるまで待機--------------------------------
            while pg.locateOnScreen(URL + r"\DataInIcon2.png", confidence=0.9) is None:
                time.sleep(1)
            ...
            while pg.locateOnScreen(URL + r"\HouteiMenu.png", confidence=0.9) is None:
                time.sleep(1)
                SKE = RPA.ImgCheck(URL, r"\SiharaiKihonEnd.png", 0.9, 10)  # 終了ダイアログ確認
                if SKE[0] is True:
                    pg.press("y")
            RPA.ImgClick(URL, r"\HouteiKousin.png", 0.9, 10)  # 法定調書更新をクリック
            # 法定調書メニューが表示されるまで待機------------------------------------
            while pg.locateOnScreen(URL + r"\HouteiAll.png", confidence=0.9) is None:
                time.sleep(1)
            # --------------------------------------------------------------------
            time.sleep(1)
            # 更新区分フラグが表示されるまで待機------------------------------------
            while pg.locateOnScreen(URL + r"\HouteiZenken.png", confidence=0.9) is None:
                pg.press("tab")
            ...
            if FC[0] is True:
                ...
                # 確認ウィンドウが表示されるまで待機-------------------------------------
                while pg.locateOnScreen(URL + r"\Rensou.png", confidence=0.9) is None:
                    time.sleep(1)
            ...
            if ErrStr == "":
                ...
                # 確認ウィンドウが表示されるまで待機-------------------------------------
                while pg.locateOnScreen(URL + r"\HouteiQ.png", confidence=0.9) is None:
                    time.sleep(1)
                # --------------------------------------------------------------------
                pg.press("y")  # yで決定(nがキャンセル)
                # 処理終了ウィンドウが表示されるまで待機----------------------------------
                while (
                    pg.locateOnScreen(URL + r"\HouteiEnd.png", confidence=0.9) is None
                ):
                    time.sleep(1)
                # --------------------------------------------------------------------
                pg.press("return")
                ...
                # 法定調書フラグが表示されるまで待機-------------------------------------
                while (
                    pg.locateOnScreen(URL + r"\HouteiFlag.png", confidence=0.9) is None
                ):
                    time.sleep(1)
                # --------------------------------------------------------------------
                print("更新完了")
