...
class ViewGUI:
    ...
    def __init__(self, window_root, w_root_top, title_n, default_path, logger):
        print("#########################################################")
        print(os.getcwd())
        print("#########################################################")
        # Controller Class生成
        ...
        self.tomlPath = self.tomlread()  # tomlパス判定格納
        ...
        logger.debug("ViewGUI起動完了")  # Log出力
        # #############################################################################
    ...
    def FrameCreate(self, NWINSize):
        ...
        try:
            ...
            self.window_sub_ctrl1.columnconfigure(0, weight=1)
            self.window_sub_ctrl1.rowconfigure(0, weight=1)
            # 画像加工
            ...
            self.window_sub_ctrl2.columnconfigure(0, weight=1)
            self.window_sub_ctrl2.rowconfigure(0, weight=1)
            # 画像加工2
            ...
            self.window_sub_ctrl4.columnconfigure(0, weight=1)
            self.window_sub_ctrl4.rowconfigure(0, weight=1)
    ...
    def FileDelete(self):
        ...
        ファイル削除ボタンクリックイベント
        """
        logger.debug("FileDelete関数起動")  # Log出力
    ...
    def event_menu(self):
        ...
        """
        # # menu作成######################################################################
        # logger.debug("SubMenuOpen関数起動")  # Log出力
        # # サブフレーム起動確認
        # try:
    ...
    def Resize(self, Resizewidth, Resizeheight):
        ...
        try:
            ...
            logger.debug("Resize関数Err")  # Log出力
            ...
            if noisepar % 2 == 0:
                ...
                if msg is True:
                    ...
                    if SCM == "画像無":
                        ...
                        logger.debug("ノイズ除去関数control側Err")  # Log出力
                        messagebox.showinfo("確認", "編集中にエラーが発生しました。")
        except:
    ...
    def event_set_folder(self):
        ...
        self.combo_file.configure(values=self.file_list)
        logger.debug(self.dir_path + "_フォルダー選択完了")  # Log出力
    ...
    def event_set_file(self):
        ...
        self.FN = os.path.splitext(os.path.basename(self.Selectfile_url))[0]
        self.str_dir.set(self.dir_path)
        self.file_list = self.control.SetDirlist(self.dir_path)

        if ".PDF" in self.FN or ".pdf" in self.FN:
            msg = messagebox.askokcancel(
                "確認", "PDFが選択されています。PNGに変換しますか？\n10ページ以上の処理は処理時間が長時間になる可能性があります。"
            )
            if msg is True:
                logger.debug("PNG変換起動")  # Log出力
                # プログレスバーの起動
                PBAR = PB.Open(tk.Toplevel())  # サブWindow作成
                spd = self.control.pdf_image(self.FN, "png", 300, PBAR)
                if spd is True:
                    f_r = 0
                    ...
                    logger.debug(self.FN + "_PNG変換完了起動")  # Log出力
                else:
                    f_r = 0
                    for f_l in self.file_list:
                        if f_l == self.FN:
                    ...
                    self.control.DrawImage("set", set_pos=set_pos)
                    self.combo_file.set(self.file_list[set_pos])
                    logger.debug(self.FN + "_PNG変換失敗起動")  # Log出力
        else:
            f_r = 0
            for f_l in self.file_list:
                if f_l == self.FN:
            ...
            self.control.DrawImage("set", set_pos=set_pos)
            self.combo_file.set(self.file_list[set_pos])
            logger.debug(self.dir_path + "_ファイル選択完了")  # Log出力
    ...
    def event_selectfile(self, event, str):
        ...
        for r in range(len(self.combo_file.values)):
            if self.combo_file.get() == self.combo_file.values[r]:
                set_pos = r
                break
        self.FN = self.control.get_file("set", set_pos=set_pos)
        self.Selectfile_url = self.FN
        if ".PDF" in self.FN or ".pdf" in self.FN:
            msg = messagebox.askokcancel(
                "確認", "PDFが選択されています。PNGに変換しますか？\n10ページ以上の処理は処理時間が長時間になる可能性があります。"
            )
            ...
            self.control.DrawImage("set", set_pos=set_pos)
            logger.debug(self.FN + "_ファイル選択後イベント完了")  # Log出力
    ...
    def event_clip_end(self, event):
        ...
        """
        print(sys._getframe().f_code.co_name, event.x, event.y)
        logger.debug("画像範囲選択イベント完了")  # Log出力
        self.event_Searchsave()  # 編集履歴判定後上書き
        if self.clip_enable:
    ...
    def event_Searchsave(self):
        ...
        print(sys._getframe().f_code.co_name)  # ターミナルへ表示
        try:
            logger.debug("OverSaveボタン起動")  # Log出力
            # 一時保存ファイルを確認
            if self.control.model.stock_url != "":
                if messagebox.askokcancel("確認", "編集履歴が残っています。上書きしますか？"):
                    os.remove(self.control.model.stock_url)
    ...
    def event_undo(self):
        ...
        """
        print(sys._getframe().f_code.co_name)  # ターミナルへ表示
        logger.debug("Undoボタンクリック起動")  # Log出力
        # 一時保存ファイルを確認
        if self.control.model.stock_url != "":
            os.remove(self.control.model.stock_url)
            self.control.model.stock_url = ""
    ...
    def LinOCROpen(self):
        ...
        """
        print(sys._getframe().f_code.co_name)  # ターミナルへ表示
        logger.debug("OCR処理起動")  # Log出力
        self.event_Searchsave()  # 編集履歴判定後上書き
        FDir = self.entry_dir.get()
        self.FN = self.combo_file.get()
        self.Imgurl = FDir + r"\\" + self.FN
        self.Imgurl = self.Imgurl.replace("/", r"\\")
        if "[select file]" in self.Imgurl:
            ...
            if self.control.model.stock_url != "":
                ...
                try:
                    ...
                    TKINTERCV2Setting.setopen(self, TKcv2)
                except:
                    self.tomlPath = self.tomlread()
                    try:
                        TKINTERCV2Setting.setopen(self, TKcv2)
                        logger.debug("OCR処理完了")  # Log出力
                    except:
                        logger.debug("OCR処理tomlファイル選択時Err")  # Log出力
            else:
                try:
                    self.tomlPath = self.tomlread()
                    TKINTERCV2Setting.setopen(self, TKcv2)
    ...
    def click_close(self):
        ...
        if messagebox.askokcancel("確認", "終了しますか？"):
            logger.debug("OCRViewClose完了")  # Log出力
...
def Open(Menu, w_root, w_root_top, url, log):
    global logger
    logger = log
    VG = ViewGUI(w_root, w_root_top, "OCR読取 Ver:0.9", url, logger)
    LoadImg(VG)
    # 　フレームループ処理
    # VG.window_root.mainloop()
...
if __name__ == "__main__":
    ...
    # Viewクラス生成
    VG = ViewGUI(main_window, "OCR読取 Ver:0.9", "./")
    LoadImg(VG)
    TKcv2 = threading.Thread(target=TKINTERCV2Setting.Main(VG))._target
    TKcv2.top.withdraw()
    TKcv2.master.withdraw()
    print("")
    # 　フレームループ処理
