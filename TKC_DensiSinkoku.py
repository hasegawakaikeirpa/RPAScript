...
# ----------------------------------------------------------------------------------------------------------------------
def SortCSVItem(
    C_Child, Col1, Col2, Col3, Col4, Key
...
def Sousin(driver, FolURL2, ItemRowArray, CSVName):
    if CSVName == "SinseiMaster":
        ...
        while pg.locateOnScreen(FolURL2 + "/SinseiSousin.png", confidence=0.9) is None:
            time.sleep(1)
            if ImgCheck(FolURL2, "TihouAnsyou.png", 0.9, 5)[0] is True:
                time.sleep(1)
                ImgClick(FolURL2, "TihouAnsyou.png", 0.9, 3)
                time.sleep(1)
                pyperclip.copy("Ha47K05121")
                pg.hotkey("ctrl", "v")  # pg日本語不可なのでコピペ
                pg.press("return")
                time.sleep(1)
                pg.press("return")
                time.sleep(1)
            if ImgCheck(FolURL2, "KokuzeiAnsyou.png", 0.9, 5)[0] is True:
                time.sleep(1)
                ...
                pg.press("return")
                time.sleep(1)
        if ImgCheck(FolURL2, "SinseiSousin.png", 0.9, 5)[0] is True:
            time.sleep(1)
            pg.press("n")
            time.sleep(1)
        ...
        ZeiresiAnsyou = ImgCheck(FolURL2, FileName, conf, LoopVal)[0]  # is True:
        if ZeiresiAnsyou is True:
            FileName = "ZeirisiAnsyou.png"
            ImgClick(FolURL2, FileName, conf, LoopVal)
            pyperclip.copy("Ha47K05121")
            pg.hotkey("ctrl", "v")  # pg日本語不可なのでコピペ
        ...
        ZeiresiAnsyouT = ImgCheck(FolURL2, FileName, conf, LoopVal)[0]  # is True:
        if ZeiresiAnsyouT is True:
            FileName = "ZeirisiAnsyouT.png"
            ImgClick(FolURL2, FileName, conf, LoopVal)
            pyperclip.copy("Ha47K05121")
            pg.hotkey("ctrl", "v")  # pg日本語不可なのでコピペ
        ...
        if ImgCheck(FolURL2, FileName, conf, LoopVal)[0] is False:
            pyperclip.copy("Ha47K05121")
            pg.hotkey("ctrl", "v")  # pg日本語不可なのでコピペ
            pg.press("return")
        # ----------------------------------------------------------------------------------------------------------------------
...
def MasterLoop(
    ...
    for x in range(C_dfRow):
        ...
        if not C_SCode == 82:
            ...
            conf = 0.9  # 画像認識感度
            LoopVal = 10
            if NoAction is False:  # 前周で操作した場合ChildCSVを再切出し
                C_Child = TaxHantei(
            ...
            if CSVName == "SinseiMaster":
                if C_Sousin == "可":
                    ItemRowArray = SortCSVItem(
                        C_Child[0], "関与先コード", "納税者(関与先)", "申請・届出書類名", "送信", C_All
                    )
                    ...
                    ErrMsg = ""
                    if (
                        ImgCheckForList(FolURL2, List, conf)[0] is True
                    ):  # リスト内の画像があればTrueと画像名を返す
                        pg.press("return")
                        ErrMsg = "送信エラー"
                        time.sleep(1)
                    # ------------------------------------------------------------------------------------------------------------
                ...
                if C_Sousin == "可":
                    ItemRowArray = SortCSVItem(
                        C_Child[0], "関与先コード", "納税者(関与先)", "税目", "送信", C_All
                    )
                    for ItemRow in ItemRowArray:
                        # Target選択の為にページダウンが必要か行数から割り出す-------------------------------------------------------------------------------------------
                        if ItemRow >= 13:  # 取得した行数が13以上なら
