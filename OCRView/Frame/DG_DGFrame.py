...
import Frame.DGMyTable as MT
import CSVOut as CSVO
import Frame.DG_ScrollableFrame as SF
...
def create_SettingFrame(self):
    # 設定メインフレーム設定--------------------------------------------------------------------
    self.Mframe = SF.ScrollableFrameDG(self.DG_Main_Frame)
    self.Mframe.grid(
...
# ####################################################################################################
# サブフレーム#########################################################################################
def create_Frame4(self, t_font):
    # ------------------------------------------------------------------------------
    # 左側メニュー
    self.AJsetOCRFrame = tk.Frame(
        self.DG_Sub_Frame, width=650, height=200, bd=2, relief=tk.RIDGE
    )  # 親フレーム
