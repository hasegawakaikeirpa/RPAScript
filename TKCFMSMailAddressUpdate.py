...
def FMSOpen(FolURL2, Lday, driver):
    try:
        ...
        time.sleep(1)
        while (
            pg.locateOnScreen(FolURL2 + "/KiridasiBox.png", confidence=0.99999) is None
        ):
            pg.press(
                [
                    "backspace",
        ...
        while (
            pg.locateOnScreen(FolURL2 + "/FileOverSTR.png", confidence=0.9) is not None
        ):
            time.sleep(1)
...
def FirstAction(FolURL2, CSVURL, ws, driver):
    ...
    for x in range(LenRow):
        ...
        TRow = CFM.CsvSortRowDouble(
            CSVURL, "関与先コード", "個人コード", wsNo, wsNoSub
        )  # CythonでC##実行
...
def FMSAction(FolURL2, wsRow, PDV):
    ...
    if wsHakkou == "メール":
        ...
        if not MAE[0] is True:
            ...
            # 履歴データベースを更新---------------------------------------------------------------------
            USQL = (
                "UPDATE m_kfmsrireki SET vc_gyou = '○' WHERE vc_FMSKnrCd = '"
                + wsKno
                + "' AND in_RrkNo_pk = "
                + str(wsin_RrkNo_pk).replace("'", "")
                + ";"
            )
