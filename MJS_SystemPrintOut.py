    ...
    try:
        if "会計大将" == Title:
            ...
            if SystemUp[0] is True:
                ...
                WriteEx = openpyxl.load_workbook(XLSURL, keep_vba=True)
                WriteExSheet = WriteEx[isnItem]
                WriteExSheet.cell(row=Ex + 5, column=Eh + 1).value = "出力履歴なし" + dt_now
                print("シート書き込み完了")
                print(WriteEx)
                WriteEx.save(XLSURL)
                WriteEx.close
                ...
                WriteEx = openpyxl.load_workbook(XLSURL, keep_vba=True)
                WriteExSheet = WriteEx[isnItem]
                WriteExSheet.cell(row=Ex + 5, column=Eh + 1).value = "決算順序未設定" + dt_now
                print("シート書き込み完了")
                print(WriteEx)
                WriteEx.save(XLSURL)
                WriteEx.close
            ...
            if SystemUp[0] is True:
                ...
                WriteEx = openpyxl.load_workbook(XLSURL, keep_vba=True)
                WriteExSheet = WriteEx[isnItem]
                WriteExSheet.cell(row=Ex + 5, column=Eh + 1).value = "物件無し" + dt_now
                print("シート書き込み完了")
                print(WriteEx)
                WriteEx.save(XLSURL)
                WriteEx.close
    ...
    if OP[0] is True:
        ...
        if PN == "申告税一覧表":
            ...
            while (
                ...
                if SRC[0] is True:
                    pg.press("return")
                SRCCCCC = ImgCheck(CFolURL, r"\Houjinzei\S_RendouCheck2.png", 0.9, 10)
                ...
                if SRCCC[0] is True:
                    pg.press("n")
                SRCCCC = ImgCheck(CFolURL, r"\Houjinzei\S_Rendou4.png", 0.9, 10)
                if SRCCCC[0] is True:
                    pg.press("y")
                # 新規別表追加選択-------------------------------------------------
    ...
    if OP[0] is True:
        ...
        while (
            ...
            if HEQ[0] is True:  # 法人番号未登録ダイアログが表示されていたら
                pg.press("y")  # yで確定
            SSL = ImgCheck(CFolURL, r"\Houjinzei\S_Rendou4.png", 0.9, 10)
    ...
    try:
        ...
        if ICFL[0] is True:  # 法人税のアイコンがあれば
            ...
            while (
                ...
                time.sleep(1)
            pg.write(str(ExRow["関与先番号_関与先番号"]))
            ...
            if ImgCheck(TFolURL, r"\NotData.png", 0.9, 10)[0] is True:
                ...
                # -----------------------------------
            # 他システムとメニューが違う-------------------------------------------------------
            if str(ExRow["関与先番号_関与先番号"]) == ThisNo:
                print("関与先あり")
            ...
            if str(ExRow["関与先番号_関与先番号"]) == ThisNo:
                ...
                if PN == "消費税確定申告書":
                    ...
                    while (
                        ...
                        KKS = ImgCheck(CFolURL, r"\KTaisyou\KihonKoushin.png", 0.9, 10)
                        if KKS[0] is True:
                    ...
                    if KKS[0] is False:
                        ...
                        while (
                            ...
                            if KN[0] is True:
                                pg.press("y")
                            DIN = ImgCheck(CFolURL, r"\KTaisyou\Din.png", 0.9, 10)
                            if DIN[0] is True:
                                pg.press("y")
                        # --------------------------------------------------------------------
                        time.sleep(1)
                    ...
                    while (
                        pg.locateOnScreen(
                            CFolURL + r"\KTaisyou\TenpuBar.png", confidence=0.99999
                        )
                        is None
                    ):
                        time.sleep(1)
                    ...
                    if NOD[0] is False:    
                        ...
                        while (
                            pg.locateOnScreen(
                                CFolURL + r"\KTaisyou\EndQ.png", confidence=0.9
                            )
                            is None
                        ):
                            time.sleep(1)
                    ...
                    SSY = ImgCheck(CFolURL, r"\Houjinzei\Sansyou_F.png", 0.9, 10)
                    if SSY[0] is True:
                    ...
                    if KJJ[0] is False:
                        ...
                        while (
                            pg.locateOnScreen(
                                CFolURL + r"\KTaisyou\NotEnd.png", confidence=0.9
                            )
                            is not None
                        ):
                            time.sleep(1)
...
# ------------------------------------------------------------------------------------------------------------------
def MainFlow(FolURL, TFolURL, CFolURL, SerchURL, XLSDir, LURL, Exlsx, driver, XLSURL):
    """
    概要: プリントメイン処理
    @param FolURL : ミロク起動関数のフォルダ(str)
    @param TFolURL : このpyファイルのフォルダ(str)
