def eTaxWebCrawler(H_id,H_pass,H_fold,H_SC,H_TN):
    ...
    H_Sikibetu_box2 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div[2]/form[1]/table/tbody/tr/td/input[2]")
    H_Sikibetu_box3 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div[2]/form[1]/table/tbody/tr/td/input[3]")
    H_Sikibetu_box4 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div[2]/form[1]/table/tbody/tr/td/input[4]") 
    #ログインPASS入力欄をxpathで取得
    H_LogPassBox = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div[2]/form[2]/table/tbody/tr/td/span[1]/input")
    H_Log_btn = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div[2]/form[2]/p/input[1]") 
    #WEB操作--------------------------------------------------------------------------------------------------------------------
...
def LogOnOuter(H_driver):
    ...
    #WEB画面要素指定------------------------------------------------------------------------------------------------------
    try: 
        H_MSG_tex = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/p[1]")#メッセージボックスの中身ページのH1を指定    
        H_MSG_Table = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/form/table/tbody/tr[1]")#メッセージボックスのテーブルをxpath取得
    except Exception:
        pass
    else:
...
def ParGet(H_driver,H_MSG_rowItem,H_MSG_row):
            try:
                #WEBTable日付
                H_MSG_TableItem1 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/form/table/tbody/tr[{}]/td[1]".format(H_MSG_rowItem + 1)).text
                H_MSG_TableItem2 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/form/table/tbody/tr[{}]/td[2]".format(H_MSG_rowItem + 1)).text
                H_MSG_TableItem3 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/form/table/tbody/tr[{}]/td[3]".format(H_MSG_rowItem + 1)).text
                #WEBTableメールタイトル                
                H_MSG_TableItem4 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/form/table/tbody/tr[{}]/td[4]".format(H_MSG_rowItem + 1)).text.replace('\u3000', ' ')
                H_MSG_TableItem5 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/form/table/tbody/tr[{}]/td[5]".format(H_MSG_rowItem + 1)).text
                H_MSG_TableItem6 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/form/table/tbody/tr[{}]/td[6]".format(H_MSG_rowItem + 1)).text
                H_MSG_TableItem7 = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/form/table/tbody/tr[{}]/td[7]".format(H_MSG_rowItem + 1)).text
        
            except Exception:
                pass
                #print(str(H_SC) + "_" + H_TN + "データ無")        
            else:
                H_result = H_driver.page_source.split("goDetail",H_MSG_row)#ページソースからgoDetail区切りをテーブル行数分行う
                H_result = H_result[H_MSG_rowItem + 1].split(">")#goDetail区切り配列からループ回数に応じたJavaScript引数を抽出
                H_OnClickPar = H_result[00].replace('(', '').replace('/', '').replace(')', '').replace('"', '')#JavaScript引数を置換して成型
                H_driver.execute_script("javascript:goDetail({})".format(H_OnClickPar))#抽出したJavaScript引数を利用して実行
                WebDriverWait(H_driver, 30).until(EC.presence_of_all_elements_located)#要素が読み込まれるまで最大30秒待つ
                H_driver.switch_to.window(H_driver.window_handles[1])#開いたタブに移動する
        #-----------------------------------------ページ内条件分岐---------------------------------------------
                H_Title = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/h2[1]").text#先頭H2タイトル取得
                return H_Title,H_MSG_TableItem1,H_MSG_TableItem2,H_MSG_TableItem3,H_MSG_TableItem4,H_MSG_TableItem5,H_MSG_TableItem6,H_MSG_TableItem7
#-----------------------------------------------------------------------------------------------------------------------------------------------------
def PrintIFS(H_Title,H_driver):
    if re.search("法定調書の提出について", H_Title):
        ...
        H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/form/p/input").click()#申告のお知らせボタンをクリック
        WebDriverWait(H_driver, 30).until(EC.presence_of_all_elements_located)#要素が読み込まれるまで最大30秒待つ
        H_driver.execute_script('window.print();')#開いたタブを印刷
        #time.sleep(4)
        WebDriverWait(H_driver, 30).until(EC.presence_of_all_elements_located)#要素が読み込まれるまで最大30秒待つ
...
def LogReturn(LogAns,H_driver):
    if LogAns == "メインメニュー":#ログイン成功の場合
        #WEB画面要素指定------------------------------------------------------------------------------------------------------
        try:
            H_MSG_OpenMyNo = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div[1]/div/div[1]/div[3]/div/form/p/input")
            H_MSG_OpenMyNo.click()
            WebDriverWait(H_driver, 30).until(EC.presence_of_all_elements_located)#要素が読み込まれるまで最大30秒待つ
            return H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/p[1]").text#メッセージボックス画面の文言を返す
        except:
            H_MSG_Open = H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/div/div[1]/div/div[1]/div[2]/form/p/input") #メッセージボックスを開くボタンをxpath指定    
            H_MSG_Open.click()
            WebDriverWait(H_driver, 30).until(EC.presence_of_all_elements_located)#要素が読み込まれるまで最大30秒待つ
            return H_driver.find_element_by_xpath("/html/body/div[1]/div[2]/p[1]").text#メッセージボックス画面の文言を返す
        ...
        try: 
            H_PassKousinBtn = H_driver.find_element_by_xpath("/html/body/div[2]/div/form[2]/input[3]")
...
def LoginLoop(H_SCode,H_TKCName,H_First,H_FirstP,H_SecondP):
    ...
    if not H_LogMSGAns == "表示するメッセージはありません。" and not H_LogMSGAns == "認証エラー":
        ...
        for H_MSG_rowItem in range(H_L_Row):#WEBテーブル行数分ループ
            ...
            if SPDF == False or SPDF == None or Hurikae == True:
