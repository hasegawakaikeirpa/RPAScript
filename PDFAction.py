...
def SerchNonPDF(URL):
    ...
    for dir_ListItem in dir_List:
        ...
        dir_Files = SerchdirFiles(Serchd)
        for dir_FilesItem in dir_Files:
            if ".pdf" not in dir_FilesItem[1] and ".xdw" not in dir_FilesItem[1]:
                # print(dir_FilesItem)
        ...
        try:
            if ".xdw" not in dir_Files[0][1]:
                if len(dir_Files) == 1:
                    ...
                    if os.path.isfile(KeyURLL) is True:
...
URL = r"\\\\Sv05121a\\e\\電子ファイル\\メッセージボックス\\2022-3\\送信分受信通知"
