...
class ColumnHeader(Canvas):
    ...
    def redraw(self, align="w"):
        ...
        self.thefont = self.table.thefont
        self.configure(
            scrollregion=(0, 0, self.table.tablewidth + self.table.x_start, self.height)
        )
        ...
        for level in levels:
            ...
            self.create_line(
                x, 0, x, self.height, tag="gridline", fill="white", width=2
            )
            i += 1
            y = y + h - 2
    ...
    def handle_left_click(self, event):
        ...
        self.draggedcol = None
        self.drawRect(
            self.table.currentcol, tag="dragrect", color="lightblue", outline="white"
        )
    ...
    def popupMenu(self, event):
        ...
        formatcommands = {
            "Set Color": self.table.setColumnColors,
        ...
        popupmenu.add_command(
            label="Set %s as Index" % colnames, command=self.table.setindex
        )
        popupmenu.add_command(label="Delete Column(s)", command=self.table.deleteColumn)
        if ismulti == True:
            popupmenu.add_command(
                label="Flatten Index", command=self.table.flattenIndex
            )
        popupmenu.add_command(label="Fill With Data", command=self.table.fillColumn)
        popupmenu.add_command(
            label="Create Categorical", command=self.table.createCategorical
        )
        popupmenu.add_command(
            label="Apply Function", command=self.table.applyColumnFunction
        )
        popupmenu.add_command(
            label="Resample/Transform", command=self.table.applyTransformFunction
        )
        popupmenu.add_command(label="Value Counts", command=self.table.valueCounts)
        popupmenu.add_command(
            label="String Operation", command=self.table.applyStringMethod
        )
        popupmenu.add_command(
            label="Date/Time Conversion", command=self.table.convertDates
        )
    ...
    def renameColumn(self):
        ...
        new = simpledialog.askstring(
            "New column name?", "Enter new name:", initialvalue=name
        )
        if new != None:
            if new == "":
    ...
    def drawRect(self, col, tag=None, color=None, outline=None, delete=1):
        ...
        rect = self.create_rectangle(
            x1, y1 - w, x2, self.height, fill=color, outline=outline, width=w, tag=tag
        )
        self.lower(tag)
        return
    ...
    def redraw(self, align="w", showkeys=False):
        ...
        if self.table.showindex == True:
            if util.check_multiindex(index) == 1:
                ind = index.values[v]
                cols = [
                    pd.Series(i).astype("object").astype(str).replace("nan", "")
                    for i in list(zip(*ind))
                ]
...
class IndexHeader(Canvas):
    ...
    def __init__(self, parent=None, table=None, width=None, height=25, bg="gray50"):
